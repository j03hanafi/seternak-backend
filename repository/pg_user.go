...
// FindByID searches for a user in the database using their unique identifier and converts the result to a domain.User.
// Returns a domain.User object or an error if the user is not found or if there's an internal error.
func (p *pgUserRepository) FindByID(ctx context.Context, uid ulid.ULID) (*domain.User, error) {
	l := logger.FromCtx(ctx)
